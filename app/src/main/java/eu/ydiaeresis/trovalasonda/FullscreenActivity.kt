...
class FullscreenActivity : AppCompatActivity(), LocationListener, MapEventsReceiver,
    ...
    private val resultLauncher =
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            ...
            if (data != null && data.extras != null) {
                ...
                if (reset) MaterialAlertDialogBuilder(
                    this,
    ...
    private fun addShowcaseItem(
        seq: MaterialShowcaseSequence,
        view: View,
        title: Int,
        content: Int,
    ): MaterialShowcaseView {
        val mcsv = MaterialShowcaseView.Builder(this@FullscreenActivity).setTarget(view)
    ...
    private fun showcase(id: String): Boolean {
        ...
        MaterialShowcaseSequence(this, id).apply {
            ...
            addShowcaseItem(
                ...
                binding.type,
                R.string.SONDE_DATA,
                R.string.TAP_HERE_TO_CHANGE_SONDE_TYPE_FREQUENCY
            ).setTarget(
                MultipleViewsTarget(
                    listOf(
                        binding.verticalSpeed,
            ...
            addShowcaseItem(
                ...
                binding.buzzer,
                R.string.BUZZER,
                R.string.MUTE_YOUR_RECEIVER_TAPPING_THIS
            )
            addShowcaseItem(
                this,
                binding.batteryMeter,
                R.string.BATTERY,
                R.string.KEEP_AN_EYE_ON_YOUR_RECEIVER_BATTERY_LEVEL
            )
            addShowcaseItem(
                this,
                binding.menuLayer,
            ...
            addShowcaseItem(
                ...
                R.string.RECEIVER_PARAMETERS,
                R.string.SET_PINS_BANDWIDTH_AND_CALIBRATION_FOR_YOUR_RECEIVER
            )
    ...
    override fun onLocationChanged(location: Location) {
        ...
        if (sondeLat != null && sondeLon != null && sondeLat != 0.0 && sondeLon != 0.0) {
            distance =
                GeoPoint(currentLocation).distanceToAsDouble(GeoPoint(sondeLat!!, sondeLon!!))
            setDistance(distance)
        }
        updateSondeDirection()

        accuracyOverlay.points =
            Polygon1.pointsAsCircle(
    ...
    private fun updateSondeLocation(id: String?, lat: Double?, lon: Double?, alt: Double?) {
        ...
        if (lat == null || lon == null || sondeLat!! == 0.0 || sondeLon!! == 0.0) return
        ...
        var maxHeight = if (burst) sondeAlt!! + 1 else site?.burstAltitude?.toDouble() ?: 33000.0
        val ascRate = site?.ascentRate?.toDouble() ?: 5.0
        val descRate = site?.descentRate?.toDouble() ?: 5.0
        if (useImperialUnits()) {
        ...
        if (sondeAlt != null && deltaAlt != null && deltaAlt != 0.0) {
            ...
            if (!burst && /*heightDelta>0 &&*/ deltaAlt < 0) {
                ...
                mkBurst?.apply {
                    ...
                    setVisible(true)
                    title = getBurstTitle(Instant.now(), alt, ascRate, descRate)
                }
                playSound(R.raw._541192__eminyildirim__balloon_explosion_pop)
            }
            heightDelta = deltaAlt
        }
        if (burst) maxHeight = sondeAlt!! + 1
        if (nPositionsReceived > 10 && (lastPrediction == null || lastPrediction?.until(
                Instant.now(),
                ChronoUnit.SECONDS
            )!! > 60)
        ) {
            lastPrediction = Instant.now()
            predict(lat, lon, alt, maxHeight, ascRate, descRate)
        }
    }
    ...
    private fun updateRSSI(rssi: Float) {
        ...
        if (timeLastSeen != null && Instant.now()
                .toEpochMilli() - timeLastSeen!!.toEpochMilli() > 8000 && sondeLat != null && sondeLon != null && !reportAlreadyShown && distance < 30
        ) showReport()
    }
    ...
    private fun showReport() {
        ...
        WebPageChooserDialog().showForRecovery(
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                if (points.isNotEmpty()) {
                    ...
                    if ((lastPrediction == null || lastPrediction?.until(
                            Instant.now(),
                            ChronoUnit.SECONDS
                        )!! > 60)
                    ) {
                        ...
                        points.last().apply {
                            val maxHeight =
                                if (burst) sondeAlt!! + 1 else site?.burstAltitude?.toDouble()
                                    ?: 33000.0
                            val ascRate = site?.ascentRate?.toDouble() ?: 5.0
                            val descRate = site?.descentRate?.toDouble() ?: 5.0
                            predict(latitude, longitude, altitude, maxHeight, ascRate, descRate)
                        }
                    }
                }
            } catch (ex: Exception) {
    ...
    private fun receiverNotConnectedWarning() {
        Snackbar.make(binding.root, R.string.RECEIVER_NOT_CONNECTED, Snackbar.LENGTH_LONG)
            .setAction("connect") { askForScanning(true) }.show()
    }
    ...
    fun getBurstTitle(t: Instant, maxHeight: Double, ascRate: Double, descRate: Double): String {
        val dtf = DateTimeFormatter.ofPattern("HH:mm")
        var txt = "time: " + LocalTime.from(t.atZone(ZoneId.systemDefault())).format(dtf)
        if (useImperialUnits()) {
            val h = maxHeight * meters
            val a = ascRate * meters
            val d = descRate * meters
            txt += "\nh:%.0fft\nasc:%.1fft/s\ndesc:%.1fft/s".format(
                h `in` feet,
                a `in` feet,
                d `in` feet
            ...
            txt += "\nh:%.1fm\nasc:%.0fm/s\ndesc:%.1fm/s".format(maxHeight, ascRate, descRate)
        }
        return txt
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        @Suppress("DEPRECATION") val td =
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) ActivityManager.TaskDescription.Builder()
                .setIcon(R.drawable.ic_launcher_foreground).build()
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            if (versionDB != null) receiver?.requestVersion()
            else runOnUiThread {
                Snackbar.make(
                    binding.root,
                    getString(R.string.cannot_access_remote_firmware_updates_database),
...
//        }
        ...
        *///////////////////////////////////////
        ...
        binding.batteryMeter.setOnClickListener {
            ...
            else if (batteryLevel != null) Snackbar.make(
                binding.root,
        ...
        val copyCoordinates = { _: View ->
            ...
            val clip =
                ClipData.newPlainText(
                    "sonde coordinates",
                    "${binding.lat.text} ${binding.lon.text}"
                )
            clipboard.setPrimaryClip(clip)
        ...
        binding.id.setOnClickListener {
            if (sondeId != null) {
                if (sondeLat == null || sondeLon == null) WebPageChooserDialog().showForInfo(
                    supportFragmentManager,
                    sondeId!!,
                    currentLocation!!.latitude,
                    currentLocation!!.longitude
                )
                else WebPageChooserDialog().showForInfo(
                    supportFragmentManager,
                    sondeId!!,
                    sondeLat!!,
                    sondeLon!!
        ...
        binding.subpanel.setOnClickListener {
            ...
            SondeTypeDialog().apply {
                ...
                type = sondeType
                sondeTypes = receiver?.sondeTypes
...
//                handler.postDelayed({updateSondeLocation("V4640201",45.7,7.1001,12000.0)},3000)
        ...
        binding.menuSettings.setOnClickListener {
            if (receiver == null) receiverNotConnectedWarning()
                ...
                if (receiver?.requestSettings() == true) showProgress(true)
                else Snackbar.make(
                    binding.root,
                    getString(R.string.settings_not_available_for_this_receiver),
                    Snackbar.LENGTH_LONG
                ).show()
            }
            closeMenu()
        ...
        binding.menuLayer.setOnClickListener {
            ...
            Snackbar.make(
                ...
                arrayOf(
                    ...
                    "CyclOSM",
                    "Mapbox satellite"
                )[mapStyle] + " " + applicationContext.getString(R.string.MAP_STYLE_SELECTED),
                Snackbar.LENGTH_SHORT
            ).show()
            with(binding.map) {
                setTileSource(
        ...
        binding.menuCenterSonde.setOnClickListener {
            ...
            else Snackbar.make(binding.root, R.string.NO_SONDE_TO_SHOW, Snackbar.LENGTH_SHORT)
                .show()
            closeMenu()
        }
        ...
        binding.menuMaps.setOnClickListener {
            if (sondeId != null) navigate(mkSonde?.position!!)
                ...
                binding.root,
                R.string.NO_SONDE_TO_NAVIGATE_TO,
                Snackbar.LENGTH_SHORT
            )
                .show()
        ...
        binding.map.run {
            ...
            addOnFirstLayoutListener { _: View, _: Int, _: Int, _: Int, _: Int ->
                ...
                mkSondehub = MyMarker(binding.map).apply {
                    icon =
                        AppCompatResources.getDrawable(applicationContext, R.drawable.ic_sonde_blue)
                    setAnchor(Marker.ANCHOR_CENTER, Marker.ANCHOR_CENTER)
                    setVisible(false)
                    setOnMarkerClickListener { marker, _ -> if (sondeId != null) navigate(marker.position); true }
...
//                        icon.setTint(Color.HSVToColor(floatArrayOf((10-i)*24F,1F,1F)))
//                        icon.setTintBlendMode(BlendMode.DARKEN)
//                        setAnchor(Marker.ANCHOR_CENTER,Marker.ANCHOR_CENTER)
//                        infoWindow=null
//                        position=GeoPoint(45.0714488, 7.593995+i*(7.649485-7.593995)/10.0)
//                    })
//                }
//                fldPollicino.add(Marker(binding.map).apply {
//                    icon=AppCompatResources.getDrawable(applicationContext,R.drawable.spot)
...
//                    setAnchor(Marker.ANCHOR_CENTER,Marker.ANCHOR_CENTER)
//                    infoWindow=null
//                    position=GeoPoint(45.076808585334724, 7.632490920227752)
//                })
//                fldPollicino.add(Marker(binding.map).apply {
//                    icon=AppCompatResources.getDrawable(applicationContext,R.drawable.spot)
...
//                    setAnchor(Marker.ANCHOR_CENTER,Marker.ANCHOR_CENTER)
//                    infoWindow=null
//                    position=GeoPoint(45.077118163302394, 7.638972082918306)
//                })
                ///////////////////////////////////////////////////
        ...
        handler.post(object : Runnable {
            override fun run() {
                ...
                if (receiver != null && (sondeId?.length
                        ?: 0) > 0 && timeLastSeen != null && timeLastSeen?.until(
                        Instant.now(),
                        ChronoUnit.MINUTES
                    )!! >= 2 && (timeLastSondehub == null || timeLastSondehub?.until(
                        Instant.now(),
                        ChronoUnit.SECONDS
                    )!! >= 30)
                ) {
                    getFromSondeHub(
                        receiver!!.sondeTypes[sondeType],
                        sondeId!!,
                        timeLastSeen!!
                    ) //crash on exit?
...
//        }
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val tawhiri = Tawhiri(
                    ...
                    lng,
                    alt,
                    if (burst) alt + 1 else max(alt + 100, maxHeight),
                    ascRate,
                    descRate
                )
                mkTarget?.setVisible(false)
                ...
                tawhiri.getPrediction().onEach {
                    if (it.stage == "descent" && !burst) mkBurst?.apply {
                        ...
                        title = getBurstTitle(t, maxHeight, ascRate, descRate)
                        setVisible(true)
                    }
                    ...
                    roadOverlay = RoadManager.buildRoadOverlay(road).apply {
                        ...
                        val duration = road.mDuration.toDuration(DurationUnit.SECONDS)
                            .toComponents { hours, minutes, _, _ ->
                                hours.toDuration(DurationUnit.HOURS) + minutes.toDuration(
    ...
    private fun navigateGeneric(position: GeoPoint) {
        val uri =
            String.format(Locale.US, "geo:%f,%f", position.latitude, position.longitude).toUri()
        val intent = Intent(Intent.ACTION_VIEW, uri)
        startActivity(intent)
    }
    ...
    override fun onRestoreInstanceState(savedInstanceState: Bundle) {
        ...
        binding.id.text = normalizeSondeId()
        if (sondeAlt != null) if (useImperialUnits()) {
    ...
    private fun askForScanning(firstTime: Boolean = false) {
        ...
        if (!lm.isProviderEnabled(LocationManager.GPS_PROVIDER))
            MaterialAlertDialogBuilder(this, R.style.MaterialAlertDialog_rounded).setIconAttribute(
                android.R.attr.alertDialogIcon
            ).setTitle(R.string.app_name)
            ...
            MaterialAlertDialogBuilder(this, R.style.MaterialAlertDialog_rounded).setCancelable(
                false
            )
                .setTitle(if (firstTime) getString(R.string.chose_connection) else getString(R.string.no_receiver_found))
                .setSingleChoiceItems(
                    arrayOf<CharSequence>(
                        getString(R.string.bluetooth_classic),
                        getString(R.string.bluetooth_low_energy),
                        getString(R.string.no_receiver)
                    ), lastConnectionChoice
                ) { _, x ->
                    choice = x
                    if (choice < 2) Snackbar.make(
                        binding.root,
                        getString(
                            if (choice == 0) R.string.connect_to_mysondygo_v2
...
//    }
    ...
    override fun onVersion(version: String) {
        ...
        if (a.size == 1) {
            ...
            platform = receiver?.getFirmwareName() ?: ""
        } else {
            this.version = a[1]
            platform = a[0]
        }
//        Log.i(TAG,"Current version: $version")
        if (!versionChecked && versionDB != null && receiver != null) {
            ...
            if (versionInfo != null && versionInfo.version != this.version) runOnUiThread {
